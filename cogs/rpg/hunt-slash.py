...
class RowButtons(disnake.ui.ActionRow):
    def __init__(self, author: disnake.Member, combat):
        ...
        for card in hand:
            ...
            if f"card_{card_num}" in combat.used_cards:
                self.add_button(style=ButtonStyle.secondary, label=card, disabled=True, custom_id=f"card_{card_num}")
            else:
                self.add_button(style=ButtonStyle.secondary, label=card, disabled=False, custom_id=f"card_{card_num}")
            card_num += 1
